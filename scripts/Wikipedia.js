...
function WIKI_Get(title) {
	...
	//"http://en.wikipedia.org/w/api.php?action=query&titles="+title+"&prop=pageimages&format=json&pithumbsize=500&callback=?"
	console.log(title);
	var img = null;
	$.ajax({
		 type: "GET",
				...
				var pages = data.query.pages;
				 if(pages!= null) {
					 for (page in pages) {
						...
						WIKI_GetImg(title, pages[page].extract)
					 }
				 }
		 },
...
function WIKI_GetImg(title, text) {
